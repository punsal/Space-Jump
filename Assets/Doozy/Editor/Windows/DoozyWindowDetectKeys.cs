...
namespace Doozy.Editor.Windows
{
    public partial class DoozyWindow
    {
        private void DetectKeys(Event current)
        {
            ...
            //Close Window
//            if (DGUI.KeyMapper.DetectKeyDownCombo(current, EventModifiers.Alt, KeyCode.X)) Close();

            //Collapse or Expand the Left Toolbar Menu
            if (DGUI.KeyMapper.DetectKeyDownCombo(current, EventModifiers.Alt, KeyCode.BackQuote)) ToggleToolbarMenuExpandOrCollapse();
