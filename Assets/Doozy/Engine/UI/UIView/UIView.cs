...
namespace Doozy.Engine.UI
{
    ...
    public class UIView : UIComponentBase<UIView>
    {
        ...
        /// <summary> Cancels an auto hide, if it was initiated </summary>
        ...
        /// <summary> Starts the loop animations set up on this UIView </summary>
        ...
        /// <summary> Stops any running loop animations on this UIView </summary>
        public void StopLoopAnimation() { UIAnimator.StopAnimations(RectTransform, AnimationType.Loop); }

        /// <summary> Toggles this UIView by showing it if it's NOT visible or hiding it otherwise </summary>
        /// <param name="instantAction"> Should the animation happen instantly? (zero seconds) </param>
...
//                    yield return new WaitForSecondsRealtime(HideBehavior.Animation.TotalDuration + UIViewSettings.DISABLE_WHEN_HIDDEN_TIME_BUFFER); //wait for seconds realtime (ignore Unity's Time.Timescale)
        ...
        {
            yield return null;                            //skip a frame
            if (EventSystem.current == null) yield break; //check that the EventSystem exists
            ...
            if (AutoSelectButtonAfterShow && SelectedButton != null)       //check that the auto select option is enabled and that a selectedButton has been referenced
                EventSystem.current.SetSelectedGameObject(SelectedButton); //select the referenced selectedButton
            else if (DeselectAnyButtonSelectedOnShow)                      //check that the deselect any button on SHOW option is enabled
                EventSystem.current.SetSelectedGameObject(null);           //clear any selection
        }
        ...
        /// <para/> If not UIView is found, it will return an empty list.
        /// </summary>
        ...
        {
            var panels = new List<UIView>(); //create temp list
            foreach (UIView panel in Database)
            {
                if (!panel.ViewCategory.Equals(viewCategory)) continue;
                if (!panel.ViewName.Equals(viewName)) continue;
                panels.Add(panel); //categories and names match -> add to list
            }

            return panels; //return list
        }

        /// <summary>
        /// Hides all the UIViews with the passed view name and the default view category name 'General'.
        ...
        /// <summary>
        /// Hides all the UIViews with the passed view category and view name.
        /// <para/> Note that if an UIView view category and view name are set to the default values, the HIDE animation will NOT play.
        /// </summary>
        ...
        /// <summary>
        /// Shows all the UIViews with the passed view name and the default view category name 'General'.
        ...
        /// <summary>
        /// Shows all the UIViews with the passed view category and view name.
        /// <para/> Note that if an UIView view category and view name are set to the default values, the SHOW animation will NOT play.
        /// </summary>
        /// <param name="viewCategory"> View category </param>
        /// <param name="viewName"> View name (found in the view category) </param>
        ...
        /// <summary> Executes the HIDE animation according to each UIView's specific settings </summary>
